...
from fully_algorithm.dataloader import DataSet, set_experiment
from fully_algorithm.GNN import MLP, Linear, CustomizedGNN
from fully_algorithm.process_data import set_prediction
...
def set_global_variables(experiment, prediction, topk, iteration, ablation_type="exclusion", verbose=True):
    ...
    set_experiment(experiment, ablation_type)
...
def save_results(args, result, type, method=None):
    averaged_f1 = calculate_mean(result)

    if type == "fully_algorithm":
        output_dir = Path("logs/Fully_Algorithm_Result.csv")

        if not output_dir.exists():
            df = pd.DataFrame(list(), columns=['Method', 'Ablation', 'Prediction', 'Top K', 'Iteration',
                                               'Averaged Result', 'Std', 'Worst', 'Median', 'Best', 'Hash'])
            df.to_csv("logs/Fully_Algorithm_Result.csv")

        csv_row = [str(method), str(ABLATION), str(PREDICTION), str(TOP_K), str(ITERATION),
        ...
        dataframe = pd.DataFrame([csv_row], columns=['Method', 'Ablation', 'Prediction', 'Top K', 'Iteration',
                                                     'Averaged Result', 'Std', 'Worst', 'Median', 'Best', 'Hash'])
        dataframe.to_csv("logs/Fully_Algorithm_Result.csv", mode='a', header=False, index=True)
...
def fully_algorithm_experiment(args, method, topk, num_iter, prediction, ablation):
    ...
    for __ in tqdm(range(num_iter), unit_scale=True, desc='Running Fully Algorithm-guided Experiments...'):
    ...
    save_results(args, result, type="fully_algorithm", method=method)


if __name__ == '__main__':
    #########################################################
    # Fully Algorithm-guided Experiment
    #########################################################

    print("Experiment: running [FULLY ALGORITHM-GUIDED] experiment.")
    ...
    for method in methods:
        for topk in topks:
            fully_algorithm_experiment(args, method, topk, num_iter=10,
                                       prediction="material_id", ablation=[])
