from fully_algorithm.process_data import *
...
from fully_algorithm.config import ABLATED_FEATURES, GLOBAL_FEATURES
...
def set_experiment(experiment, ablation_type):
    global ABLATED_FEATURES, ABLATION_TYPE
    ...
    for graph in tqdm(data, desc="Encoding Features"):
        ...
        if ABLATION_TYPE == "exclusion":
            ...
            if len(features_to_include) != 0:
                print("Error in performing ablation (ablation type: exclusion)!")
                exit(1)

        else:
            print("Error: wrong type of ablation experiment!")
            exit(1)
